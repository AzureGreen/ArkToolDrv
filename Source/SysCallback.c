...
GetPspLoadImageNotifyRoutineAddress()
{
	...
	*/
	...
	GetNtosExportVariableAddress(L"PsSetLoadImageNotifyRoutine", (PVOID*)&PsSetLoadImageNotifyRoutine);
...
{
	UINT_PTR PspLoadImageNotifyRoutine = GetPspLoadImageNotifyRoutineAddress();
	...
	else
	{
		...
		for (i = 0; i < 64; i++)		// ???? 理论上数组大小应该只有8才对
		{
			...
			if (MmIsAddressValid((PVOID)(PspLoadImageNotifyRoutine + i * sizeof(UINT_PTR))))
			{
				...
				if (NotifyItem > g_DynamicData.KernelStartAddress &&
					MmIsAddressValid((PVOID)(NotifyItem & ~7)))
				{
					...
					if (CallbackAddress && MmIsAddressValid((PVOID)CallbackAddress))
					{
						...
						{
							sci->Callbacks[CurrentCount].Type = NotifyLoadImage;
							sci->Callbacks[CurrentCount].CallbackAddress = CallbackAddress;
							sci->Callbacks[CurrentCount].Description = NotifyItem;
						}
...
GetPspCreateThreadNotifyRoutineAddress()
{
	...
	*/
	...
	GetNtosExportVariableAddress(L"PsSetCreateThreadNotifyRoutine", (PVOID*)&PsSetCreateThreadNotifyRoutine);
...
{
	UINT_PTR PspCreateThreadNotifyRoutine = GetPspCreateThreadNotifyRoutineAddress();
			...
			if (MmIsAddressValid((PVOID)(PspCreateThreadNotifyRoutine + i * sizeof(UINT_PTR))))
			{
					...
					MmIsAddressValid((PVOID)(NotifyItem & ~7)))
				{
					...
					if (CallbackAddress && MmIsAddressValid((PVOID)CallbackAddress))
					{
						...
						if (NumberOfCallbacks > CurrentCount)
						{
							sci->Callbacks[CurrentCount].Type = NotifyCreateThread;
							sci->Callbacks[CurrentCount].CallbackAddress = CallbackAddress;
							sci->Callbacks[CurrentCount].Description = NotifyItem;
						}
						sci->NumberOfCallbacks++;
...
{
	UINT_PTR CallbackList = GetCallbackListHeadAddress();

	DbgPrint("%p\r\n", CallbackList);

	if (!CallbackList)
	{
		DbgPrint("CallbackListAddress NULL\r\n");
		return FALSE;
	}
	else
	{
		PCM_NOTIFY_ENTRY	Notify = NULL;
		PLIST_ENTRY			NotifyListEntry = (PLIST_ENTRY)(*(PUINT_PTR)CallbackList);	// Flink
		...
		*/
		...
		do 
		{
			...
			{
				if (MmIsAddressValid((PVOID)(Notify->Function)) && Notify->Function > g_DynamicData.KernelStartAddress)
				{
					UINT_PTR CurrentCount = sci->NumberOfCallbacks;
					if (NumberOfCallbacks > CurrentCount)
					{
						sci->Callbacks[CurrentCount].Type = NotifyCmpCallBack;
						sci->Callbacks[CurrentCount].CallbackAddress = Notify->Function;
						sci->Callbacks[CurrentCount].Description = Notify->Cookie.QuadPart;
					}
					sci->NumberOfCallbacks++;
				}
			}
			NotifyListEntry = NotifyListEntry->Flink;
		} while (NotifyListEntry != ((PLIST_ENTRY)(*(PUINT_PTR)CallbackList)));
	}
...
GetKeBugCheckCallbackListHeadAddress()
{
	...
	*/
	...
	GetNtosExportVariableAddress(L"KeRegisterBugCheckCallback", (PVOID*)&KeRegisterBugCheckCallback);
...
{
	UINT_PTR KeBugCheckCallbackListHead = GetKeBugCheckCallbackListHeadAddress();
			...
			TravelListEntry = TravelListEntry->Flink)
		{
			...
			if (Dispatch && MmIsAddressValid((PVOID)Dispatch))
			{
				...
				if (NumberOfCallbacks > CurrentCount)
				{
					sci->Callbacks[CurrentCount].Type = NotifyKeBugCheck;
					sci->Callbacks[CurrentCount].CallbackAddress = Dispatch;
					sci->Callbacks[CurrentCount].Description = (UINT_PTR)TravelListEntry;
				}
				sci->NumberOfCallbacks++;
			}
		}		
...
GetKeBugCheckReasonCallbackListHeadAddress()
{
	...
	*/
	...
	GetNtosExportVariableAddress(L"KeRegisterBugCheckReasonCallback", (PVOID*)&KeRegisterBugCheckReasonCallback);
...
{
	UINT_PTR KeBugCheckReasonCallbackListHead = GetKeBugCheckReasonCallbackListHeadAddress();
		...
		*/
			...
			TravelListEntry = TravelListEntry->Flink)
		{
			...
			if (Dispatch && MmIsAddressValid((PVOID)Dispatch))
			{
				...
				if (NumberOfCallbacks > CurrentCount)
				{
					sci->Callbacks[CurrentCount].Type = NotifyKeBugCheckReason;
					sci->Callbacks[CurrentCount].CallbackAddress = Dispatch;
					sci->Callbacks[CurrentCount].Description = (UINT_PTR)TravelListEntry;
				}
				sci->NumberOfCallbacks++;
...
GetIopNotifyShutdownQueueHeadAddress()
{
	...
	*/
	...
	GetNtosExportVariableAddress(L"IoRegisterShutdownNotification", (PVOID*)&IoRegisterShutdownNotification);
...
{
	UINT_PTR IopNotifyShutdownQueueHead = GetIopNotifyShutdownQueueHeadAddress();
			...
			DispatchListEntry = DispatchListEntry->Flink)
		{
			...
			if (Address && MmIsAddressValid((PVOID)Address))
			{
				...
				if (DeviceObject && MmIsAddressValid((PVOID)DeviceObject))
				{
					...
					if (NumberOfCallbacks > CurrentCount)
					{
						sci->Callbacks[CurrentCount].Type = NotifyShutdown; 
						sci->Callbacks[CurrentCount].CallbackAddress = GetShutdownDispatch(DeviceObject);		// 通多设备对象找到驱动对象 SHUTDOWN 派遣例程
						sci->Callbacks[CurrentCount].Description = (UINT_PTR)DeviceObject;
					}
					sci->NumberOfCallbacks++;
				}
			}
...
EnumSysCallbackNotify(OUT PVOID OutputBuffer, IN UINT32 OutputLength)
{
	...
	UINT32	NumberOfCallbacks = (OutputLength - sizeof(SYS_CALLBACK_INFORMATION)) / sizeof(SYS_CALLBACK_ENTRY_INFORMATION);		// Ring3传递Length

	bOk = GetLoadImageCallbackNotify(sci, NumberOfCallbacks);		// 映像加载 卸载回调
	bOk = GetCreateThreadCallbackNotify(sci, NumberOfCallbacks); // 创建线程回调
	bOk = GetRegisterCallbackNotify(sci, NumberOfCallbacks);     // 注册表回调
	bOk = GetBugCheckCallbackNotify(sci, NumberOfCallbacks);     // 错误检查回调
	bOk = GetBugCheckReasonCallbackNotify(sci, NumberOfCallbacks);// 
	bOk = GetShutDownCallbackNotify(sci, NumberOfCallbacks);     // 关机回调
	...
	if (NumberOfCallbacks >= sci->NumberOfCallbacks)
	{
